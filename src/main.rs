...
mod utils;
pub use utils::{TrimConfig, InputConfig};
use utils::*;
...
#[command(
    ...
    about = "SegPE: a simple program for classifing and separating PE/SE FASTQ after removing adapters via adapter-index seq",
...
fn process_reads_se(
    ...
    three_artificial_map: &BTreeMap<Vec<u8>, String>,
    five_index_map: &BTreeMap<Vec<u8>, String>,
    three_idx_map: &BTreeMap<Vec<u8>, String>,
    five_artificial_id_map: &BTreeMap<String, Vec<u8>>,
    three_artificial_id_map: &BTreeMap<String, Vec<u8>>,
    five_art_seed_map: &BTreeMap<String, Vec<(String,u8)>>,
    three_art_seed_map: &BTreeMap<String, Vec<(String,u8)>>,
...
fn process_reads_pe(
    ...
    let start = Instant::now();
    let idx_loc = input_config.idx_loc;
    let merge_pe_bool = input_config.merge_pe;
    // ...existing code...
    let (mut reads1, mut reads2) = pair;
    if merge_pe_bool {
        let merged_result = merge_pe((reads1.clone(), reads2.clone()), align_config.seed_len,
                              align_config.error_tolerance, align_config.match_score, align_config.error_score, align_config.gap_open_score, align_config.gap_extend_score);
...
fn pretrain(pretrain_reads_number: usize, five_art_length: usize, five_artificial_idx_map: BTreeMap<Vec<u8>, String>,
    ...
    input_config: &InputConfig, align_config: &AlignConfig, trim_config: &TrimConfig) -> Result<BTreeMap<Vec<u8>, String>, std::io::Error> {
...
fn seg_pe(
    ...
    // Extract configs from input_config
    let num_threads = input_config.num_threads;
    let batch = input_config.batch;
    let outdir = &input_config.outdir;
    // let merge_pe_bool = input_config.merge_pe;
    ...
    // 检查PE2文件是否存在
    let pe2_exists = !input_config.pe2_fastq.is_empty() && Path::new(&input_config.pe2_fastq).exists();
    
    // 打开FASTQ文件
    let file1 = File::open(&input_config.pe1_fastq)?;
    
    // 获取file1 prefix_name
    let prefix1 = outdir.to_string() + "/" + &get_fastq_prefix(&input_config.pe1_fastq);
    
    // 仅在PE2存在时获取file2和prefix2
    let file2_option = if pe2_exists {
        Some(File::open(&input_config.pe2_fastq)?)
    } else {
        None
    };
    
    let prefix2 = if pe2_exists {
        outdir.to_string() + "/" + &get_fastq_prefix(&input_config.pe2_fastq)
    } else {
        String::new()
    };
    ...
    for _ in 0..num_threads {
        ...
        let trim_config = trim_config.clone();
        let align_config = align_config.clone();
        let input_config = input_config.clone(); // CLONE HERE
        // 根据模式创建不同类型的处理线程
        if pe2_exists {
            let handle = thread::spawn(move || {
                for pair in rx.lock().unwrap().iter() {
                    ...
                    process_reads_pe(&five_artificial_idx_map, &three_artificial_map, &five_index_map, &three_index_map,
                        &five_artificial_id_map, &three_artificial_id_map, &five_art_seed_map, &three_art_seed_map,
                        five_art_length, &prefix1_clone, &prefix2_clone, pair, io_tx_clone.lock().unwrap().clone(), &input_config, &align_config, &trim_config);
                }
            });
            handles.push(handle);
        } else {
            let handle = thread::spawn(move || {
                for reads1 in rx.lock().unwrap().iter() {
                    process_reads_se(&five_artificial_idx_map, &three_artificial_map, &five_index_map, &three_index_map,
                        &five_artificial_id_map, &three_artificial_id_map, &five_art_seed_map, &three_art_seed_map,
                        five_art_length, &prefix1_clone, reads1.0, io_tx_clone.lock().unwrap().clone(), &input_config, &align_config, &trim_config);
                }
            });
            handles.push(handle);
        }
    ...
    // 为不同的输入格式优化读取策略
    let reader1: Box<dyn std::io::BufRead> = if input_config.pe1_fastq.ends_with(".gz") || input_config.pe1_fastq.ends_with(".gzip") {
    ...
    if pe2_exists {
        ...
        let reader2: Box<dyn std::io::BufRead> = if input_config.pe2_fastq.ends_with(".gz") || input_config.pe2_fastq.ends_with(".gzip") {
...
fn main() {
    ...
    // five_art_length is not a field of InputConfig, so store it separately
    // let mut five_art_length = 0;
    let input_config = InputConfig {
        five_art_fa: args.five_art_fa.clone(),
        three_art_fa: args.three_art_fa.clone(),
        five_idx_fa: args.five_idx_fa.clone(),
        three_idx_fa: args.three_idx_fa.clone(),
        idx_loc: args.idx_loc,
        pe1_fastq: args.pe1_fastq.clone(),
        pe2_fastq: args.pe2_fastq.clone(),
        outdir: args.outdir.clone(),
        num_threads: args.num_threads,
        batch: args.batch,
        train: args.train,
        merge_pe: args.merge_pe,
    };

    let proceed_result = proceed_artidx_map(&input_config.five_art_fa, &input_config.three_art_fa, &input_config.five_idx_fa, &input_config.three_idx_fa, args.seed_len);
    if let Ok((fa_length, five_artificial_idx_map, five_index_map, three_artificial_map, three_index_map, five_artificial_id_map, three_artificial_id_map, five_art_seed_map, three_art_seed_map)) = proceed_result {
        let five_art_length = fa_length;

        let trim_config = TrimConfig {
            qual_trim: args.qual_trim,
            n_trim: args.n_trim,
            length_offset: args.length_offset,
            ascii_offset: args.quality_ascii_offset,
            poly_trim: if args.poly_trim > 0 { Some(args.poly_trim) } else { None },
        ...
        let pretrain_result = pretrain(
            args.train, five_art_length, five_artificial_idx_map, five_index_map.clone(),
            three_artificial_map.clone(), three_index_map.clone(), five_artificial_id_map.clone(), three_artificial_id_map.clone(),
            five_art_seed_map.clone(), three_art_seed_map.clone(), &input_config, &align_config, &trim_config);
        
        match pretrain_result {
            Ok(five_artificial_idx_map_ext) => {
                match seg_pe(
                    five_art_length, five_artificial_idx_map_ext, five_index_map, three_artificial_map, three_index_map,
                    five_artificial_id_map, three_artificial_id_map, five_art_seed_map, three_art_seed_map, &input_config, &align_config, &trim_config) {
